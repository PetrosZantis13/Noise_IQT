...
def plot_omegas():
    ...
    Om_list = np.linspace(1, 30, 51)*KHZ
    T2s = []
    #g_factor *= 1/20  # with pairing
    
    for nu_c in nu_c_list:
        for Om in Om_list:
            SBtot = em.SB(nu_c, dzB, g_factor, nuSE, SBa, SV, SA)
            T2 = em.compute_T2(SBtot)
            tgate = em.compute_tgate(nu_c, dzB, Om, 1)
            T2s.append(T2/tgate)
    

    T2s = np.array(T2s).reshape((nu_c_list.size, Om_list.size))

#     plt.plot(nu_c_list/KHZ, T2s.T[0])
#     plt.xlabel("Secular frequency (kHz)")
#     plt.ylabel("T2* (s)")
#     plt.show()
#     
#     plt.plot(Om_list/KHZ, T2s[0])
#     plt.xlabel("Rabi power (kHz)")
#     plt.ylabel("T2* (s)")
#     plt.show()

    plt.contourf(Om_list/KHZ, nu_c_list/KHZ, T2s)
    plt.ylabel("Secular frequency (kHz)")
    plt.xlabel("Rabi power (kHz)")
    plt.colorbar(label="T2* / tgate ratio")
    plt.show()
